...
class CPLSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CPL
        fields = ['id','kode', 'deskripsi', 'kategori', 'cpmks']
...
class MatakuliahSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Matakuliah
        fields = ['id','kode', 'nama', 'sks', 'semester', 'cpls']
    
    def get_cpls(self, obj):
        # Get all CPLs associated with this matakuliah through MatakuliahCPL
        matakuliah_cpls = MatakuliahCPL.objects.filter(matakuliah=obj).select_related('cpl')
